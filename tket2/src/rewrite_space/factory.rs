    ...
    fn explore(
        ...
        while let Some((node, walker)) = queue.pop_front() {
            for (pattern_match, new_walker) in self.find_pattern_matches(node, walker) {
                ...
                let Some(replacement) = self.get_replacement(&pattern_match, host) else {
                    continue;
                };
                let Ok(new_commit) = new_walker.try_create_commit(
                    pattern_match.matched_wires().cloned(),
                    replacement,
                    |node, port| self.map_boundary(node, port, &pattern_match, host),
                ) else {
                    continue;
                };
...
impl IterMatchedWires for Vec<PersistentWire> {
    fn matched_wires(&self) -> impl Iterator<Item = &PersistentWire> + '_ {
        self.iter()
    }
}
...
mod tests {
    ...
    fn test_cz_commit_factory(perhugr1: PersistentHugr) {
        ...
        dbg!(all_rewrites.iter().map(|(rw, _)| rw).collect_vec());

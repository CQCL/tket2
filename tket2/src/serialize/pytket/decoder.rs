...
pub struct Tk1DecoderContext<'h> {
    ...
    pub hugr: DFGBuilder<&'h mut Hugr>,
...
impl<'h> Tk1DecoderContext<'h> {
    /// Initialize a new [`Tk1Decoder`], using the metadata from a
    /// [`SerialCircuit`].
    ///
    ...
    #[expect(unused)] // TEMPORARY, until we remove the todo below.
    pub fn try_new(
        ...
        let mut dfg: DFGBuilder<&mut Hugr> = todo!(); // FunctionBuilder::with_hugr(hugr, name, sig).unwrap();
    ...
    ///
    /// After this call, the HUGR passed to the [`Tk1DecoderContext::try_new`] constructor
    /// will contain the fully defined function.
    pub fn finish(mut self) -> Result<BuildHandle<DfgID>, Tk1ConvertError> {

...
use super::{METADATA_B_OUTPUT_REGISTERS, METADATA_B_REGISTERS};
...
pub struct BitTracker<N> {
    ...
    bit_to_reg: HashMap<Wire<N>, RegisterUnit>,
...
impl<N: HugrNode> BitTracker<N> {
    ...
    pub fn add_bit_register(&mut self, wire: Wire<N>) -> &RegisterUnit {
        let reg = self
            .unused_registers
            .pop_front()
...
impl<N> Default for BitTracker<N> {
    fn default() -> Self {
        Self {
            inputs: Default::default(),
            outputs: Default::default(),
            bit_to_reg: Default::default(),

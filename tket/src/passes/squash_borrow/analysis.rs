...
use crate::resource::{
    CopyableValueId, DefaultResourceFlow, OpValue, ResourceFlow, ResourceId, ResourceScope,
    ResourceScopeConfig, UnsupportedOp,
};
use crate::Circuit;
...
impl<N: HugrNode> BorrowInfo<N> {
    ...
    /// Prefer using [Self::try_from_borrow_node] or [Self::try_from_return_node].
    ///
    ...
    fn try_from_ports(
        ...
        let borrow_index_id = circuit
            .get_opvalue(node, borrow_index)
            .and_then(|v| v.as_copyable())
            .expect("valid port");
...
impl<H: HugrView> ResourceScope<H> {
    fn get_value_definition(&self, copyable: CopyableValueId) -> Option<(H::Node, OutgoingPort)> {
        let mut all_out_value_ports = self
            .nodes()
            .iter()
            .flat_map(|&n| self.hugr().out_value_types(n).map(move |(p, _)| (n, p)));
        all_out_value_ports.find(|&(n, p)| {
            self.get_opvalue(n, p).expect("valid port") == OpValue::Copyable(copyable)
        })
    }

    fn as_const(&self, copyable: CopyableValueId) -> Option<&constant::Value> {
        let (def_node, def_port) = self.get_value_definition(copyable)?;
    ...
    fn test_borrow_flow(inline_borrow_analysis: BorrowAnalysis) {
        ...
        for resource_start in circuit.hugr().node_outputs(circuit.input_node()) {
            let resource_id = scope
                .get_opvalue(circuit.input_node(), resource_start)
                .and_then(|v| v.as_resource())
                .unwrap();

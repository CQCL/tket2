...
use crate::resource::{CopyableValueId, Interval, InvalidInterval, ResourceId, ResourceScope};
...
pub struct Subcircuit<N = hugr::Node> {
    ...
    input_copyable_values: Vec<CopyableValueId>,
...
impl<N: HugrNode> Subcircuit<N> {
    ...
    pub fn try_from_nodes(
        ...
        for node in nodes {
            update_intervals(&mut intervals, node, circuit);
    ...
    pub fn try_extend(
        ...
        let output_copyable_values = circuit.get_copyable_values(node, Direction::Outgoing);
...
fn update_copyable_inputs<N: HugrNode>(
    input_copyable_values: &mut Vec<CopyableValueId>,
    node: N,
    circuit: &ResourceScope<impl HugrView<Node = N>>,
) -> Result<(), InvalidSubcircuit<N>> {
    for copyable_input in circuit.get_copyable_values(node, Direction::Incoming) {
    ...
    if circuit
        .get_copyable_values(node, Direction::Outgoing)
        .count()
        > 0
    {
        return Err(InvalidSubcircuit::OutputCopyableValues(node));
...
/// Update the intervals to include the given node.
fn update_intervals<N: HugrNode>(
...
impl<N: HugrNode> Subcircuit<N> {
    ...
    fn extend_copyable_inputs(
        ...
        let input_copyable_values = circuit.get_copyable_values(node, Direction::Incoming);
        let mut was_changed = false;

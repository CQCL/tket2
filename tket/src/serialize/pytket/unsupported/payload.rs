...
/// Identifier for a hyper edge between unsupported subgraphs, or between an
/// unsupported subgraph and a HUGR input/output node.
///
/// These are tracked in the opaque barrier [`UnsupportedSubgraphPayload`] in a pytket circuit, so
/// we can reconstruct the edges with types that are not encoded as pytket register dependencies.
#[derive(
    Debug,
    derive_more::Display,
    Default,
...
#[serde(transparent)]
pub struct SubgraphHyperEdge(u64);

impl SubgraphHyperEdge {
...
pub struct UnsupportedSubgraphPayload {
    ...
    inputs: Vec<(Type, SubgraphHyperEdge)>,
    ...
    /// The types can also be inferred from the encoded hugr or linked
    /// subcircuit, but we store them here to be robust.
    outputs: Vec<(Type, SubgraphHyperEdge)>,
}
...
impl UnsupportedSubgraphPayload {
    ...
    pub fn new<N: HugrNode>(
        ...
        for subgraph_inputs in subgraph.incoming_ports() {
            ...
            let edge_id = SubgraphHyperEdge::new(input_wire);
        ...
        let outputs = subgraph
            .outgoing_ports()
            .iter()
            .map(|(n, p)| SubgraphHyperEdge::new(Wire::new(*n, *p)));
    ...
    pub fn inputs(&self) -> impl Iterator<Item = (&Type, SubgraphHyperEdge)> + '_ {
        self.inputs.iter().map(|(t, e)| (t, *e))
    }

    /// Returns the outputs types and internal edge IDs of the payload.
    pub fn outputs(&self) -> impl Iterator<Item = (&Type, SubgraphHyperEdge)> + '_ {

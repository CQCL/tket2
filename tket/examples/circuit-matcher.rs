...
use tket::{
    ...
    rewrite::matcher::{CircuitMatcher, MatchContext, MatchOutcome, MatchingOptions},
    ...
    fn match_tket_op<H: HugrView>(
        ...
        match op {
            ...
            Rx | Ry | Rz => {
                ...
                let Some(half_turns) = match_context.circuit.as_const_f64(op_args[1]) else {
                    trace_debug_info("=> Could not determine angle of rotation\n");
                    return MatchOutcome::default().skip(MatchInfo { num_cx, debug_info });
                };
                const EPS: f64 = 1e-6;
...
fn main() {
    ...
    for (i, (subcircuit, info)) in matches.into_iter().enumerate() {
        println!("==== Match {} ====\n{}", i, info.debug_info);
        println!("  Num CX: {}", info.num_cx);
        println!(
            "  Matched nodes: {:?}\n",
            subcircuit.nodes(&circuit).sorted().collect_vec()
        );
    }
}

...
impl PyQubitOpBeforeArg {
    ...
    fn __repr__(&self) -> String {
        format!("QubitOpBeforeArg({})", self.index)
    }
}
...
impl PyQubitOpAfterArg {
    ...
    fn __repr__(&self) -> String {
        format!("QubitOpAfterArg({})", self.index)
    }
}
...
impl<'py> IntoPyObject<'py> for PyOpArg {
    ...
    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        Ok(match self.0 {
            OpArg::Qubit(index) => PyQubitOpArg {
                index: index.0 as usize,
            }
            .into_pyobject(py)?
            .into_any(),
            OpArg::QubitOpBefore(index) => PyQubitOpBeforeArg {
                index: index.0 as usize,
            }
            .into_pyobject(py)?
            .into_any(),
            OpArg::QubitOpAfter(index) => PyQubitOpAfterArg {
                index: index.0 as usize,
            }
            .into_pyobject(py)?
            .into_any(),

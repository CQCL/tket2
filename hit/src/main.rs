...
use commands::{
    CheckoutCommand, Command, CommandExecutor, ExtractBestCommand, LoadCommand, LogCommand,
    RunCommand, ShowCommand,
};
use constants::HITFILES_DIR;

#[derive(Parser)]
#[command(name = "hit")]
#[command(about = "A CLI tool for testing/running RewriteSpace constructs")]
struct Cli {
    #[command(subcommand)]
    command: CliCommands,
}
...
enum CliCommands {
    ...
    Run {
        ...
        explorer: String,
        /// Maximum number of rewrites to explore
        #[arg(long)]

...
pub static DEFAULT_FROM_HALFTURNS_ERROR: std::sync::LazyLock<ConstError> =
...
#[allow(dead_code)]
pub static DEFAULT_ROTATION_EXTENSION: std::sync::LazyLock<
    RotationCodegenExtension<DefaultPreludeCodegen>,
> = std::sync::LazyLock::new(|| RotationCodegenExtension::new(DefaultPreludeCodegen));
...
impl<PCG: PreludeCodegen> CodegenExtension for RotationCodegenExtension<PCG> {
    ...
    {
        builder
            .custom_type((ROTATION_EXTENSION_ID, ROTATION_TYPE_ID), |ts, _| {
                Ok(llvm_angle_type(&ts).into())
            })
...
mod test {
    ...
    // The name used in `SimpleHugrConfig` at
    // https://github.com/CQCL/hugr/blob/4e6b6d8df576f43bd5ced51b2eb4f1ed4d5c3b82/hugr-llvm/src/emit/test.rs#L154
    const SIMPLE_HUGR_MAIN: &str = "main";

    #[rstest]
    #[case::default_prelude(0, DefaultPreludeCodegen)]
    #[case::qis_prelude(1, QISPreludeCodegen)]
    fn emit_all_ops(
        #[case] _id: i32,
    ...
    fn exec_aadd(
        ...
        let half_turns = exec_ctx.exec_hugr_f64(hugr, SIMPLE_HUGR_MAIN);
    ...
    fn exec_to_halfturns(
        ...
        let halfturns = exec_ctx.exec_hugr_f64(hugr, SIMPLE_HUGR_MAIN);
    ...
    fn exec_from_halfturns(
        ...
        let r = exec_ctx.exec_hugr_f64(hugr, SIMPLE_HUGR_MAIN);
